...
model_cf = setup_model.find({"model_name": "CF"}, {
...
scheduler.add_job(id='my_job', func=save_model1,
...
def savemodelsa():
    ...
    if support_values and metric and confidence_values and base_on:
        ...
        df1["Gender"] = df1["Gender"].apply(
            lambda x: x.lower())  # setup lower case text
        ...
        print(old_data)
        # append data to previouse data
        old_data = old_data.append(df1, ignore_index=True)

        old_data['age_group'] = old_data['Age'].apply(
            lambda x: '10-20' if x <= 20 else '21-40' if x <= 40 else '41-60' if x <= 60 else '60+')
        ...
        try:
            for age_group, data in age_groups:
                for gender, gender_data in data.groupby('Gender'):
                    ...
                    te_ary = te.fit(gender_data['Store']).transform(
                        gender_data['Store'])
                    df_temp = pd.DataFrame(te_ary, columns=te.columns_)

                    # Run the Apriori algorithm to find frequent item sets
                    frequent_itemsets = apriori(
                        df_temp, min_support=support_values/100, use_colnames=True)

                    # Generate association rules from the frequent item sets
                    rules = association_rules(
                        frequent_itemsets, metric=metric, min_threshold=confidence_values/100)
            ...
            filename = "association" + \
                now.strftime("%Y_%m_%d_%H_%M_%S") + ".pkl"
